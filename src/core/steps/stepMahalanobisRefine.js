...
export function stepMahalanobisRefine(intervals, text, context) {
    ...
    const { mean, cov, threshold } = context.mahalStats || {};
    if (!mean || !cov || threshold === undefined) {
        logger.warn('未找到完整的全局统计（均值和阈值），回退到原片段');
        return intervals;
    }
    logger.debug(`使用全局阈值: ${threshold?.toFixed(4) || '未设置'}`);
    ...
    for (let idx = 0; idx < intervals.length; idx++) {
        ...
        const sequences = findStableHighSequences(distances, threshold);
